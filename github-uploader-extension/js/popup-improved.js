...
class GitHubUploader {
   ...
   */
  async processDirectoryEntry(dirEntry, fileList, basePath = '') {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory) {
        await this.processDirectoryEntry(entry, fileList, path);
      } else {
        const file = await new Promise((resolve, reject) => {
          entry.file(resolve, reject);
        });
        // Add webkitRelativePath-like property
        file.relativePath = `${dirEntry.name}/${path}`;
        fileList.push(file);
      }
    }
  }
  ...
  handleUploadComplete(results, errors) {
    ...
    if (results.length > 0) {
      ...
      successDiv.innerHTML = '<h4>Successfully uploaded:</h4>';
